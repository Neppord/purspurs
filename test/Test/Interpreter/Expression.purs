...
import PursPurs.Value (empty_scope, insert) as Value

simple_eval :: String -> Value Expr
simple_eval expr = evaluate_expr Value.empty_scope (parse_expression expr)
  ...
  it "applyes Constructor" do
    let
      ...
      env = Value.empty_scope #
        ( Value.insert "Foo"
            $ ValueCallable
            $ CallableLambda "$0" Value.empty_scope
...
foreign_ = describe "handle foregin" do
  it "calls foregin functions" do
    ...
    evaluate_expr (Value.empty_scope # Value.insert "inc" inc) (parse_expression "inc 42")
